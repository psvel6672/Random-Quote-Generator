...
export class AppComponent implements OnInit {
  ...
  generateQuote(){
    let tmpLength = this.newQuote.length;
    // console.log(tmpLength);
    let randomNum = this.getRandom(0, tmpLength);
    this.QuoteToday = String(this.newQuote[Number(randomNum)]['quote']);
    this.writtenBy = "- "+String(this.newQuote[Number(randomNum)]['source']);
    // console.log(randomNum+", Quote :: "+this.QuoteToday);
  }
  ...
  CopyQuote() {
    let copyGfGText = document.querySelector('[class="randomQuote"]');
    this.getQuoteText = String(copyGfGText?.querySelector('p')?.innerText);
    navigator.clipboard.writeText(String(this.getQuoteText));
    console.log(this.getQuoteText);
  }
